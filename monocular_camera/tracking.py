...
def update_or_deregister(objects, vehicles, distance):
    ...
    intervehicular_distance, deleted_counts = 0, 0

    for index in sorted(indexes_to_be_deleted, reverse=True):
        if vehicles[index].buffer == 0:
            vehicles[index].exit_time = time.time()
            intervehicular_distance = vehicles[index].distance-vehicles[index-1].distance
            deleted_counts += 1
            del vehicles[index]
        else:
            vehicles[index].buffer -= 1

    return intervehicular_distance, deleted_counts
